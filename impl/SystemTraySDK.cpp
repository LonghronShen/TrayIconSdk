...
// icon list
//- Anton Treskunov 						 Added the
// virutal CustomizeMenu() method - Anton Treskunov
//
...
#include <codeproject/ui/tray_icon/SystemTraySDK.h>
...
#ifndef AfxIsValidString
static BOOL inline AfxIsValidString(LPCWSTR lpsz, UINT nLength = -1) {
  if (lpsz == NULL)
    return FALSE;
  return ::IsBadStringPtrW(lpsz, nLength) == 0;
}

static BOOL inline AfxIsValidString(LPCSTR lpsz, UINT nLength = -1) {
  if (lpsz == NULL)
    return FALSE;
...
ATOM CSystemTray::RegisterClass(HINSTANCE hInstance, LPCTSTR &wndClassName) {
  WNDCLASSEX wcex;

  wndClassName = TRAYICON_CLASS;
...
BOOL CSystemTray::Create(HINSTANCE hInst, HWND hParent, UINT uCallbackMessage,
  ...
  auto atom = this->RegisterClass(hInst, wndClassName);
  ASSERT(atom != NULL);

  // Create an invisible window
  m_hWnd = ::CreateWindow(wndClassName, _T(""), WS_POPUP, CW_USEDEFAULT,
                          CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                          hParent ? hParent : NULL, 0, hInst, 0);
...
BOOL CSystemTray::ShowBalloon(LPCTSTR szText, LPCTSTR szTitle /*=NULL*/,
...
HWND CSystemTray::GetNotificationWnd() const { return m_tnd.hWnd; }
...
UINT CSystemTray::GetCallbackMessage() const { return m_tnd.uCallbackMessage; }
...
LRESULT CSystemTray::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam,
                             BOOL &bHandled) {
  auto nIDEvent = wParam;
...
LRESULT CSystemTray::OnTaskbarCreated(UINT uMsg, WPARAM wParam, LPARAM lParam,
                                      BOOL &bHandled) {
  InstallIconPending();
  return 0L;
}
...
#ifndef _WIN32_WCE
LRESULT CSystemTray::OnSettingChange(UINT uMsg, WPARAM wParam, LPARAM lParam,
                                     BOOL &bHandled) {
  auto uFlags = wParam;
  auto lpszSection = (LPCTSTR)lParam;
...
LRESULT CSystemTray::OnTrayNotification(UINT uMsg, WPARAM wParam, LPARAM lParam,
                                        BOOL &bHandled) {
...
using MessageHandler = LRESULT (CSystemTray::*)(UINT uMsg, WPARAM wParam,
                                                LPARAM lParam, BOOL &bHandled);

BOOL inline HandleMessage(MessageHandler handler, CSystemTray *obj,
                          std::function<bool()> matched, UINT message,
                          WPARAM wParam, LPARAM lParam, LRESULT &result) {
  if (matched()) {
    BOOL handled;
    auto foo = std::bind(handler, obj, _1, _2, _3, _4);
    result = foo(message, wParam, lParam, handled);
    return handled;
  }
  return FALSE;
}
...
LRESULT PASCAL CSystemTray::WindowProc(HWND hWnd, UINT message, WPARAM wParam,
                                       LPARAM lParam) {
  // The option here is to maintain a list of all TrayIcon windows,
  ...
  if (HandleMessage(
          &CSystemTray::OnTaskbarCreated, pTrayIcon,
          [&]() { return message == CSystemTray::m_nTaskbarCreatedMsg; },
  ...
  // Animation timer
  if (HandleMessage(
          &CSystemTray::OnTimer, pTrayIcon,
          [&]() {
            return message == WM_TIMER && wParam == pTrayIcon->GetTimerID();
          },
          message, wParam, lParam, result)) {
    return result;
  }
  ...
  // Settings changed
  if (HandleMessage(
          &CSystemTray::OnSettingChange, pTrayIcon,
          [&]() {
            return message == WM_SETTINGCHANGE &&
                   wParam == pTrayIcon->GetTimerID();
          },
          message, wParam, lParam, result)) {
    return result;
  }
  ...
  if (HandleMessage(
          &CSystemTray::OnTrayNotification, pTrayIcon,
          [&]() { return message == pTrayIcon->GetCallbackMessage(); }, message,
...
BOOL CSystemTray::GetW2K() {
  OSVERSIONINFO os = {sizeof(os)};
  GetVersionEx(&os);
  return (VER_PLATFORM_WIN32_NT == os.dwPlatformId && os.dwMajorVersion >= 5);
}

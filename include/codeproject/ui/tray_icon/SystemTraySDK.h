...
public:
  ...
  CSystemTray(HINSTANCE hInst, HWND hParent, UINT uCallbackMessage,
              LPCTSTR szTip, HICON icon, UINT uID, BOOL bhidden = FALSE,
  ...
  // Operations
public:
  BOOL inline Enabled() { return m_bEnabled; }
  BOOL inline Visible() { return !m_bHidden; }

  // Create the tray icon
  BOOL Create(HINSTANCE hInst, HWND hParent, UINT uCallbackMessage,
              LPCTSTR szTip, HICON icon, UINT uID, BOOL bHidden = FALSE,
              LPCTSTR szBalloonTip = NULL, LPCTSTR szBalloonTitle = NULL,
              DWORD dwBalloonIcon = NIIF_NONE, UINT uBalloonTimeout = 10);
  ...
  HWND inline GetSafeHwnd() const { return m_hWnd; }
  UINT_PTR inline GetTimerID() const { return m_nTimerID; }
...
public:
  static LRESULT PASCAL WindowProc(HWND hWnd, UINT message, WPARAM wParam,
                                   LPARAM lParam);
...
protected:
  ...
  virtual ATOM RegisterClass(HINSTANCE hInstance, LPCTSTR &wndClassName);
...
public:
  ...
  virtual LRESULT OnTrayNotification(UINT uMsg, WPARAM wParam, LPARAM lParam,
                                     BOOL &bHandled);
